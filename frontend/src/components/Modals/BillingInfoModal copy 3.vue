        ...
        <div class="p-6">
           ...
           <!-- Shared Wrapper (Optional) -->
            ...
            <div class="max-w-5xl mx-auto bg-white rounded-xl shadow p-6 mt-6 space-y-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button class="flex-none px-4 py-2 my-2 rounded-lg duration-300 text-sm bg-blue-500 text-white">
            ...
            <div v-if="showUpdateBilling" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8 text-sm text-gray-700">
                    ...
                    <!-- File Upload Students (spans 2 columns) -->
                      <div class="sm:col-span-2">
                        <label class="block font-semibold text-gray-600 mb-1">Upload Trainees</label>
                       ...
                       <table class="min-w-full border-collapse text-xs">
                        ...
                        <tbody>
                            ...
                            <td class="border px-4 py-2">
                                <!-- Status Dropdown -->
                                <select v-model="row.status" class="w-full border rounded px-2 py-1 text-xs">
           ...
           <template v-for="(item, index) in sortedBPeriods" :key="item.id">
            ...
            >
                ...
                <div class="col-span-2">
                    <div class="font-semibold text-gray-600">Trainees:</div>
                      ...
                      <table class="min-w-full border-collapse text-xs">
                        <thead class="bg-gray-100 sticky top-0 z-10">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">Middle Name</th>
                                <th class="px-4 py-2 border">SG Number</th>
                                <th class="px-4 py-2 border">Training Cost</th>
                                <th class="px-4 py-2 border">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(row, index) in item.list_trainees" :key="index">
                                <td class="border px-4 py-2 text-center">{{ index + 1 }}</td>
                                <td class="border px-4 py-2">
                                ...
                                <td class="border px-4 py-2">
                                    <span class="w-full text-center text-xs">{{ row.status }}</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        // }
            ...
            .then((res2)=>{
                ...
                if(res2.status == 201){
                    ...
                    if(this.student_rows){

                        var new_data = [];

                        for(var i = 0; i < this.student_rows.length; i++){
                            new_data.push({
                                fname : this.student_rows[i].fname,
                                mname : this.student_rows[i].mname,
                                lname : this.student_rows[i].lname,
                                extension : this.student_rows[i].extension,
                                sg_number : this.student_rows[i].sg_number,
                                training_cost : this.student_rows[i].payment,
                                ctrl_num : this.data.ctrl_num,
                                bt_id : this.billing_data.bt_id.id,
                                status : this.student_rows[i].status,
                            })
                        }

                        axios.post(process.env.VUE_APP_BASE_URL + '/trainees/add', new_data)
                        .then((res)=>{
                            console.log(res);
                        });
                    }
