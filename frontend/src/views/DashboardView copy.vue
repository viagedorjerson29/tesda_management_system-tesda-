<template>
    <div class="flex flex-col p-5 gap-4 h-full">
        ...
        <div v-show="$store.state.user.ut_id.description == 'Admin'" class="flex items-center gap-2">
            <select v-model="training_center_id" @change="getStats" class="rounded-lg text-gray-800">
        ...
        <div class="flex-none grid grid-cols-1 sm:grid-cols-6 gap-4" v-show="$store.state.user.ut_id.description == 'Admin'">
            <div class="flex items-center bg-green-800 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_peso.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col">
                    <p class="text-sm">Approved Budget</p>
                    <b class="text-2xl font-thin">{{ data_statistics ? formatPeso(data_statistics.total_payment) : '-' }}</b>
                </div>
            </div>

            <div class="flex items-center bg-green-700 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_peso.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col">
                    <p class="text-sm">Utilized Budget</p>
                    <b class="text-2xl font-thin">{{ data_statistics ? formatPeso(data_statistics.total_utilized) : '-' }}</b>
                </div>
            </div>

            <div class="flex items-center bg-red-600 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_peso.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col"> 
                    <p class="text-sm">Unutilized Budget</p>
                    <b class="text-2xl font-thin">{{ data_statistics ? formatPeso(data_statistics.total_unutilized) : '-' }}</b>
                </div>
            </div>

            <div class="flex items-center bg-blue-700 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_bill.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col">
                    <p class="text-sm">Trainings Implemented</p>
                    <b class="text-2xl font-thin">{{ data_statistics ? data_statistics.total_implemented : '-' }}</b>
                </div>
            </div>

            <div class="flex items-center bg-orange-700 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_bill.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col">
                    <p class="text-sm">Bills Submitted</p>
                    <b class="text-2xl font-thin">{{ data_statistics ? data_statistics.total_submitted : '-' }}</b>
                </div>
            </div>
            ...
            <div class="flex items-center bg-orange-700 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_bill.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col">
                    <p class="text-sm">Pending Bills</p>
                    <b class="text-2xl font-thin">{{ data_statistics ? data_statistics.total_pending : '-' }}</b>
                </div>
            </div>

            <!-- <div class="flex items-center bg-yellow-600 text-white rounded-lg p-5 gap-4 animate__slideInUp">
                <img src="../assets/icon_bill.png" class="flex-none h-8 w-8"/>
                <div class="flex-auto flex flex-col">
                    <p class="text-sm">Incoming TIP</p>
                    <b class="text-2xl">7</b>
                </div>
            </div> -->
        </div>
        ...
        <div>
            ...
            <div>
                <FullCalendar :options="calendarOptions" class="bg-white p-4 h-auto rounded-md animate__slideInUp"/>
            </div>
        ...
        <div class="flex-none gap-4" v-show="$store.state.user.ut_id.description == 'Admin'">

        <div class="p-4 bg-white rounded-md animate__slideInUp">
            ...
            <div class="content-center h-full">
                ...
                <canvas id="bar-chart-PE" style="max-width: 100%; height: 400px;"></canvas>
            </div>
        </div>
        ...
        <!--BUDGET UTILIZATION-->
        <div class="flex-none grid grid-cols-1 sm:grid-cols-2 gap-4" v-show="$store.state.user.ut_id.description == 'Admin'">
             ...
             <div class="flex items-center gap-2 flex-wrap">
                ...
                >
                    ...
                    <div class="content-center h-full">
                        <canvas id="bar-chart-budget-utilization" width="400" height="300"></canvas>
                    </div>
        ...
        <!--DROPPED TRAINEES-->
         <div class="flex-none grid grid-cols-1 sm:grid-cols-2 gap-4" v-show="$store.state.user.ut_id.description == 'Admin'">
             ...
             <div class="p-4 bg-white rounded-md animate__slideInUp">
                ...
                <select
                    v-model="selectedYear_Graph_DTL"
                ...
                <select v-model="selectedTC_DTL" @change="getLineChartData" class="rounded-lg text-gray-800 w-48 text-sm">
                    <option v-for="i in alltraining_center" :key="i.id" :value="i.id">
                        {{  i.description }}
                    </option>
                </select>
                ...
                <div class="content-center h-full">
                  <canvas id="line-chart-dropped-trainees" width="400" height="300"></canvas>
                </div>
             ...
             <div class="p-4 bg-white rounded-md animate__slideInUp">
                ...
                <select v-model="selectedTC_DTD" @change="getDoughnutChart" class="rounded-lg text-gray-800 w-48 text-sm">
                    <option v-for="i in alltraining_center" :key="i.id" :value="i.id">
                        {{  i.description }}
                    </option>
                </select>
        ...
        <div class="flex-none gap-4" v-show="$store.state.user.ut_id.description == 'Admin'">
         ...
         <div class="p-4 bg-white rounded-md animate__slideInUp">
            ...
            <div class="content-center h-full">
                <canvas id="bar-chart-BS" width="400" height="300"></canvas>
            </div>
        </div>
...
export default{
    ...
    mounted(){
        ...
        if(this.$store.state.user.ut_id.description == 'Admin'){
            this.getAllTrainingCenter();
        }
        else if(this.$store.state.user.ut_id.description == 'TC Staff'){
            this.getStats();
        }
        else if(this.$store.state.user.ut_id.description == 'PO Staff'){
            // this.getCalendarBillingPeriods(currentMonth);
        }
    },
    methods: {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      this.instance_chart_AR = new Chart(ctx, {
        ...
        data: {
          labels,
          datasets: [
            { label: "Competent", data: competentData, backgroundColor: "#4CAF50" },
            { label: "NYC", data: nycData, backgroundColor: "#FFEB3B" },
        ...
        options: {
          ...
          onClick: async (e, elements) => {
            ...
            const element = elements[0]; // Only first element
    ...
    console.error("Error fetching assessment rates:", err);
            ...
            try{
                ...
                .then((res) => {
                    ...
                    if (res.status == 200) {
                        ...
                        this.training_center_id = res.data[0].id;
                        this.selectedTC_DTL = res.data[0].id;
                        this.selectedTC_DTD = res.data[0].id;
                        this.getStats();
                        this.getCharts();
        ...
        try {
            const res = await axios.get(this.$store.state.user.ut_id.description == 'Admin' ? process.env.VUE_APP_BASE_URL + "/billing_periods/get_all_by_month/" + month : process.env.VUE_APP_BASE_URL + "/billing_periods/get_all_by_month_tc/" + month + "/" + this.$store.state.user.tc_id.id);
            if (res.status === 200) {
        ...
        getStats(){
            ...
            axios.get(process.env.VUE_APP_BASE_URL + '/billing_periods/get_statistics/' + (this.$store.state.user.ut_id.description == 'Admin' ? this.training_center_id : this.$store.state.user.tc_id.id) + '/' + this.selectedYear)
            .then((res) => {
                ...
                if (selected.qualifications && selected.qualifications.length > 0) {
                  ...
                  selected.qualifications.forEach(qual => {
                    ...
                    qual.ctrl_nums.forEach(ctrl => {
                      modalContent.push(`  o ${ctrl.ctrl_num}: ${ctrl.count}`);
                    });
                  });
                }
    ...
    this.instance_chart_BU = new Chart(ctx, {
      ...
      options: {
        ...
        onClick: async (e, elements) => {
          ...
          // Calculate score for insights — here we can use utilization % as score
          const score = allocated[index] > 0 ? utilizedRaw[index] / allocated[index] : 0;

          // Fetch description/insight dynamically
          const insight = await this.getInsights(score, this.selectedType_BU == 'scholarships' ? 'Budget Utilization Rate for Training Centers' : 'Budget Utilization Rate for Scholarships');
        ...
        if (res.status === 200) {
            ...
            this.instance_chart_BS = new Chart(ctx, {
                ...
                data: {
                    labels,
                    datasets: [
                        { label: 'On Time', data: onTimeData, backgroundColor: 'green' },
                        { label: 'Late', data: lateData, backgroundColor: 'red' }
                    ]
                },
                options: {
                    responsive: true,
                    plugins: { legend: { position: 'top' }, tooltip: { mode: 'index', intersect: false } },
                    scales: { y: { beginAtZero: true, title: { display: true, text: 'Number of Submissions' } } },
                    onClick: async (e, elements) => {
                        if (elements.length > 0) {
            ...
            options: {
              ...
              onClick: async (e, elements) => {
                if (elements.length > 0) {
                  ...
                  const insight = await this.getInsights(selected.WPS, 'Weighted Performance Score');
        ...
        plugins: {
          ...
          title: {
            ...
            text: this.selectedType_BA === 'scholarships'
              ? 'Budget Utilization Rate (BUR) by Scholarship'
