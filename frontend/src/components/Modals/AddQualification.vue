<template>
    ...
    <div class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="flex items-center sm:items-center justify-center min-h-full px-8 py-2 sm:p-0 animate__slideInUp" ref="modal">
            ...
            <div class="p-6">
                ...
                <div class="my-2">
                    <p class="text-sm text-gray-600 mb-2 flex items-center">Category</p>
        
                    <select v-model="data.category_id" class="font-bold border-gray-300 w-full rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
                        <option v-for="i in allcategories" :id="i.id" :key="i.id" :value="i.id">
                            {{ i.description }}
                        </option>
                    </select>
                </div>
                ...
                <div class="my-2">
                    <p class="text-sm text-gray-600 mb-2 flex items-center">Training Hour/s <span class="text-xs text-red-500 ml-2">{{ errors.hrs }}</span></p>
        
                    <input type="number" v-model="data.hrs" class="font-bold border-gray-300 w-full sm:w-1/4 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
                </div>

                 <div class="my-2">
                    <p class="text-sm text-gray-600 mb-2 flex items-center">Training Day/s <span class="text-xs text-red-500 ml-2">{{ errors.hrs }}</span></p>
        
                    <input type="number" v-model="data.days" class="font-bold border-gray-300 w-full sm:w-1/4 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
                </div>
    ...
    export default{
        ...
        methods: {
            findData(){
                axios.get(process.env.VUE_APP_BASE_URL + '/qualifications/find/' + this.data.id)
                .then((res)=>{
            ...
            confirmAction(){
                ...
                if(this.action == "add"){
                    ...
                    if(valid){
                        axios.post(process.env.VUE_APP_BASE_URL + '/qualifications/add', this.data)
                        .then((res)=>{

                            if(res.status == 201){

                                    this.showNotification(1,"Success","Qualification added!");
                                    this.refreshData();
                                    this.closeModal();
                    ...
                    axios.post(process.env.VUE_APP_BASE_URL + '/qualifications/edit', this.data)
                    .then((res)=>{

                        if(res.status == 201){
                            
                            this.showNotification(1,"Success","Qualification updated!");
                            this.refreshData();
                            this.closeModal();
            ...
            getCategories(){
                ...
                .then((res)=>{
                    ...
                    if(res.status == 200){
                        this.allcategories = res.data.sort(function(a, b){
